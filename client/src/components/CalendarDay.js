...
function CalendarDay(props) {
    ...
    return (
        <div class={`${props.d.active ? '' : 'inactive'} ${today ? 'calendar__day today' : 'calendar__day'}`}>
            <span class="calendar__date">{props.d.day}</span>
            <span class={`${today === true ? 'calendar__task calendar__task--today' : 'calendar__task'}`}>  
            {colors.map((color) => (
                <span class={`${props.d.active ? '' : 'opacity-50'} ${color} end-0 block size-3.5 rounded-full ring-2 ring-white dark:ring-neutral-900`}></span>           
            ))}
            </span>
        </div>
    )
