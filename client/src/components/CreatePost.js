...
const CreatePost = ({ onCloseModal }) => {
  ...
  useEffect(() => {
    ...
    return (
        <div id="hs-notifications" className={`hs-overlay size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto`}>
          <div className="mt-[100px] hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0  ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto">
            <div className="relative flex flex-col bg-white border shadow-sm rounded-xl overflow-hidden dark:bg-neutral-900 dark:border-neutral-800">
              ...
              <div className="p-4 sm:p-10 overflow-y-auto">
                ...
                <div className="space-y-4">
                  <div className="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-neutral-900 dark:border-neutral-800">
                    <select  onChange={(e) => setHid(e.target.value)} className="py-3 px-4 pe-9 block w-full bg-gray-100 border-transparent rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-700 dark:border-transparent dark:text-neutral-400 dark:focus:ring-neutral-600">
                        <option defaultValue="">Choose a Hobby Community to Post to</option>
                        {hobbies.map((hobby, index) => (
                            <option key={index} value={hobby._id}>{hobby}</option>
                        ))}
                    </select>
                  </div> 
              ...
              <div className="flex justify-end items-center gap-x-2 py-3 px-4 bg-gray-50 border-t dark:bg-neutral-950 dark:border-neutral-800">
                ...
                <button onClick={submitPost} className="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none" href="#">
                    Create Post
                </button>
              </div>
            </div>
